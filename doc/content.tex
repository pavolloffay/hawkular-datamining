...
interface SubscriptionManager {
    ...
    paratemeters best describing training data \cite{hyndman-state-space}. Optimization criterions can be: mean
    squared error, mean absoulute error and likelihood. These criterions basically computes errors produced by ahead
    ...
    Nelder-Mead simplex, multi-directional and bound optimization by quadratic approximation. These algorithms
    ...
    and R implementation. This section addes execution time of Data Mining implementation. Complete execution times are
    listed in \ref{tab:datamining-perf}. Execution times of Data Mining's optimizers are in some cases faster than R.
    Quality of the produced forecasts is discussed in \ref{evaluation}.
    ...
    Hawkular is packaged as a modified Wildfly server with several Hawkular components
    ...
    change. The final and implemented solution injectings Metrics as CDI bean. This aproach assumes that Metrics
...
%                \scalebox{0.33}[0.24]{\includegraphics[angle=0]{img/hawkular-explorer.pdf}}
                ...
                \caption{Explrer tab in Hawkular UI.}
    ...
    \section{REST APIs}
    The Data Mining REST API is designed to operate on the forecasting engine. When the module is deployed into Hawkular
    main interaction with Data Mining is goes through Inventory REST API and metric data are collected from JMS topic.
    But this is not limited and Data Mining offers alternatives REST endpoints which can be used in standalone
    daployment.

    REST API provide following capabilities:

    \begin{itemize}
        \item Create subscription\,--\,enable prediction.
    ...
    Accurate and more concrete REST API documentation is available on Hawkular web page\footnote{Available at
    \url{http://www.hawkular.org/}}.
    ...
    describing the integration was not proposed. The desing of the integraion was understood differently
    by both parties, what lead to reimplementing of some parts. Therefore it is better to preciselly define the
    integration iterface with other modules or projects.
